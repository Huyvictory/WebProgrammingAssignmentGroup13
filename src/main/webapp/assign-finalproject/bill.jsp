...
try{
	...
	ResultSet rs2=st.executeQuery("select *from usermodels inner join cart  where usermodels.email='"+email+"' and cart.status='bill'");
	while (rs2.next()){
%>
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs1=st.executeQuery("select *from cart inner join products  where cart.product_name = products.name and cart.email='"+email+"' and cart.status='bill'");
  while(rs1.next()){
	  sno=sno+1;
  %>
  <tr>
